    ...
    <div class="min-h-screen bg-[var(--background-color)] flex">
        ...
        <aside
            :class="[
                ...
                'bg-[var(--menu-bg)] border-r border-[var(--border-color)]',
                sidebarCollapsed ? 'w-16' : 'w-64',
                uiStore.sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0',
            ]"
        >
            <!-- TOP: Logo & Title (stacked) -->
            <div class="flex flex-col items-center justify-center px-3 py-5 border-b border-[var(--border-color)]">
          ...
          <span
              ...
              class="text-sm font-bold text-[var(--heading-color)] text-center leading-tight"
          >
                ...
                <router-link
                    ...
                    :class="[
                        isActiveRoute(item.path)
                            ...
                            : 'text-[var(--menu-text)] hover:bg-[var(--primary-color)]/5',
                ...
                <div v-if="visibleSettingsItems.length">
                    ...
                    >
                        ...
                        <ChevronDownIcon
                            ...
                            :class="[settingsOpen ? 'rotate-180' : '', isSettingsActive ? 'text-[var(--primary-color)]' : 'text-[var(--menu-text)]']"
                        />
                    </button>
                    ...
                    <div v-if="!sidebarCollapsed && settingsOpen" class="mt-1 ml-4 space-y-0.5 border-l border-[var(--border-color)] pl-3">
                        <router-link
                            ...
                            :class="[
                                route.name === sub.routeName
                                    ? 'bg-[var(--primary-color)]/10 text-[var(--primary-color)]'
                                    : 'text-[var(--menu-text)]/70 hover:bg-[var(--primary-color)]/5'
                            ]"
                        >
            ...
            <div class="border-t border-[var(--border-color)] p-3 flex items-center justify-between">
                <div v-if="!sidebarCollapsed" class="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <div class="min-w-0">
                        <p class="text-sm font-medium text-[var(--menu-text)] truncate">{{ authStore.user?.name }}</p>
                        <p class="text-xs text-[var(--menu-text)]/60 truncate">
                ...
                <button
                    ...
                    class="p-1.5 rounded-md hover:bg-[var(--primary-color)]/10 text-[var(--menu-text)] transition-colors"
        ...
        >
            <!-- Header -->
            <header class="h-16 bg-[var(--surface-color)] border-b border-[var(--border-color)] flex items-center justify-between px-4 sm:px-6 shrink-0">
                <button @click="uiStore.toggleSidebar" class="p-2 rounded-lg hover:bg-[var(--primary-color)]/10 lg:hidden">
                    <Bars3Icon class="w-6 h-6 text-[var(--text-primary)]" />
                </button>

                <div class="flex items-center gap-4 ml-auto">
                    <Menu as="div" class="relative">
                        <MenuButton class="flex items-center gap-2 p-1.5 rounded-lg hover:bg-[var(--primary-color)]/10 transition-colors">
                            <AppAvatar :name="authStore.user?.name" size="sm" bgColor="var(--primary-color)" />
                            <ChevronDownIcon class="w-4 h-4 text-[var(--text-secondary)]" />
                        </MenuButton>
                        <transition
                        ...
                        >
                            <MenuItems class="absolute right-0 mt-2 w-48 bg-[var(--surface-color)] rounded-xl shadow-elevation-2 border border-[var(--border-color)] py-1 z-50">
                                <MenuItem v-slot="{ active }">
                                    <router-link
                                        to="/settings/change-password"
                                        :class="[active ? 'bg-[var(--primary-color)]/10' : '', 'block px-4 py-2 text-sm text-[var(--text-primary)]']"
                                    >
                                        Change Password
                                    </router-link>
                                </MenuItem>
...
const settingsItemsList = [
    ...
    { name: 'Global Design',   path: '/settings/global-design',   routeName: 'GlobalDesign',     permission: null, icon: PaintBrushIcon },
